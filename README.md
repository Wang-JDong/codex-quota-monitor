...
Codex Quota Monitor 是一个面向个人用户的轻量级信息监控产品：每小时检查固定的 X 可信来源，只在出现“Codex 额度已重置、正在重置或将要重置”的明确证据时推送飞书消息。它不使用付费 X API，并且为低配、已承载其他服务的 VPS 设计。

> 当前状态：`0.1.0`，可用于单机自托管。本项目与 OpenAI、X 和飞书无隶属或背书关系。
...
- 飞书业务通知、健康告警与恢复通知。
- 可审计的本地状态、dry-run，以及 `delivery-resolve` / `health-resolve` 人工核对。
