...
Codex Quota Monitor 是一个面向个人用户的轻量级信息监控产品：每小时检查固定的 X 可信来源，只在出现“Codex 额度已重置、正在重置、将要重置或已发放可保存重置次数”的明确证据时推送飞书消息。它不使用付费 X API，并且为低配、已承载其他服务的 VPS 设计。

> 当前状态：`0.1.1`，可用于单机自托管。本项目与 OpenAI、X 和飞书无隶属或背书关系。
...
- 快：默认每小时自动检查，无需手动巡视。
- 准：固定信任白名单，并要求 Codex、额度/使用限制和重置状态同时成立。
...
- 识别已完成、进行中、未来计划和 `banked reset` 四类额度事件；可保存重置次数使用独立文案，不会被描述成当前额度已经自动恢复。
...
- 飞书业务通知、健康告警与恢复通知。
- 可审计的本地状态、dry-run、历史漏报帖幂等重处理，以及 `delivery-resolve` / `health-resolve` 人工核对。
...
```mermaid
flowchart LR
    T["systemd timer<br/>每小时"] --> R["一次性 runner"]
