...
> 当前状态：`0.2.1`，可用于单机自托管。本项目与 OpenAI、X 和飞书无隶属或背书关系。
...
当前免费 X 适配器以 `UserTweets` 读取四个账号的顶层原创帖，并以 `UserMedia` 补充可能被时间线漏掉的带图片/视频帖子（尤其是引用帖）；两个结果按 X status ID 去重。每条补充条目仍必须通过白名单作者与 status URL 双重校验。转发、引用正文和回复不作为证据；`UserTweetsAndReplies` 当前返回 HTTP 404，只有上游恢复兼容且相关行为在测试保护下通过后，才能启用专门的回复监控。
- 飞书业务通知、健康告警与恢复通知。
